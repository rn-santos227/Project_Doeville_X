    ...
    if (!path.empty()) {
      ...
      if (cacheIt == scriptFunctionCache.end()) {
        ...
        if (persistentFunctionCache.getData(path, raw)) {
          ...
          record(Keys::LUA_SET_NETWORK_CONNECTION);
      ...
      for (const auto& func : functions) {
        ...
        } else if (func == Keys::LUA_SET_NETWORK_CONNECTION) {
          entity->registerLuaFunction(func, LuaBindings::lua_setNetworkConnection, this);
